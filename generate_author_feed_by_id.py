...
def collect_all_authors_batch(program_ids):
    """Zbierz listę wszystkich autorów (dla trybu --all-authors)"""
    authors_dict = {}
    ...
    for i, program_id in enumerate(program_ids, 1):
        ...
        for episode in episodes:
            ...
            for member in team:
                ...
                author_email = member.get("id", "").strip()
...
def build_episodes_cache(program_ids):
    """Zbuduj cache wszystkich odcinków (dla trybu --all-authors)"""
    cache = {}
...
def collect_episodes_by_author_optimized(author_email, program_ids, all_episodes_cache, max_total_episodes=100, include_exclusive=False):
    """Zbierz odcinki autora używając cache (dla trybu --all-authors)"""
    author_episodes = []
...
Przykłady użycia:
  ...
  # Tryb batch - feedy dla wszystkich autorów:
  %(prog)s --all-authors --min-episodes 10
  %(prog)s --all-authors --list-only -p config.txt
  ...
  %(prog)s -p config.txt
    ...
    parser.add_argument('--all-authors', action='store_true', help='Generuj feedy dla wszystkich autorów (batch)')
    parser.add_argument('--min-episodes', type=int, default=3, help='Minimalna liczba odcinków dla wygenerowania feedu (domyślnie: 3, tylko z --all-authors)')
    parser.add_argument('--output-dir', default='feeds/authors', help='Katalog wyjściowy dla feedów autorów (domyślnie: feeds/authors/, tylko z --all-authors)')
    parser.add_argument('--list-only', action='store_true', help='Tylko wyświetl listę autorów (nie generuj feedów, tylko z --all-authors)')
    ...
    # TRYB BATCH: --all-authors (generuj feedy dla wszystkich autorów)
    if args.all_authors:
        print("\n" + "="*80)
        print("TRYB BATCH - Generowanie feedów dla wszystkich autorów")
        print("="*80 + "\n")
        
        # Zbuduj cache odcinków
        episodes_cache = build_episodes_cache(program_ids)
        
        # Zbierz listę autorów
        if args.authors_file:
                ...
                for prog_id, cache_data in episodes_cache.items():
                    for episode in cache_data['episodes']:
                        ...
                        for member in team:
                            if not member:
                                continue
                            member_email = (member.get("id") or "").strip()
                            member_name = (member.get("name") or "").strip()
            ...
            # Zbierz wszystkich autorów
            authors = collect_all_authors_batch(program_ids)
    ...
    # KROK 2: Ustal ID autora (tryb pojedynczy)
